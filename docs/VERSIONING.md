...
The toolkit is the pair of workspace crates `hfx` and `hfx-cli`
(the `hfx-cli` crate installs a binary named `hfx`). They move in
...
crates.io currently has the 0.3.0 toolkit crates, published under the crates' pre-rename names — the first
...
hfx-cli 0.4.0 implements HFX spec 0.3.0; the published 0.3.0 crates
...
|---|---|---|
| toolkit 0.2.0 / 0.1.26 (crates.io, pre-rename names) | legacy 0.1-era format | superseded |
| toolkit 0.3.0 (crates.io, pre-rename names) | HFX spec 0.2.1 | released |
| hfx 0.4.0 / hfx-cli 0.4.0 | HFX spec 0.3.0 | planned |
...
3. Verify that `hfx-cli`'s exact `=X.Y.Z` dependency pin on `hfx`
   picked up the new version — the bump script rewrites the pin, but check it
   rather than trusting it:
   `grep '^hfx =' crates/hfx-cli/Cargo.toml`.
...
7. A human maintainer creates and pushes the tag `vX.Y.Z` (agents never create
   or push tags).
8. Publish `hfx` first, wait for the crates.io index to pick it up, then
   publish `hfx-cli`. Publishing is human-fired.
