...
MUST report identical `flow_dir` and `flow_acc` values at each shared cell — the
entries MUST be windows cut from a single coherent D8 fabric, not independently
