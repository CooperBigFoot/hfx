...
No admitted pairing is a fatal non-coincidence error. Orientation never consults source vertex order for a connected reach. On the preserved TDX-Hydro corpus the first vertex of every proven reach is its outlet, which is the opposite of the vertex-order convention earlier adapter revisions trusted for near-degenerate reaches; the `orient` report records the measured polarity so a reviewer can check it per basin.
