...
No admitted pairing is a fatal non-coincidence error. Orientation never consults source vertex order for a connected reach. On the preserved TDX-Hydro corpus the first vertex of every proven reach is its outlet, which is the opposite of the vertex-order convention earlier adapter revisions trusted for near-degenerate reaches. This is measured evidence, not an adopted convention: the `orient` report records the polarity histogram so a reviewer can check it per basin.
...
For a healthy native root with predecessors, the root's upstream endpoint is collected before downstream traversal, so predecessor order cannot change the result. A predecessor with exactly one admitted pairing names the root endpoint it drains into, and so does any predecessor endpoint that coincides exactly with a root endpoint. One named endpoint is the root's upstream end and the opposite endpoint is the outlet (`root-predecessor-proven`). Evidence naming both root endpoints is fatal wherever it is consulted. When the evidence names no endpoint, a predecessor that matches both ends of the root cannot be conditioned and is fatal. An earlier inherited convention remains for the residual case in which every predecessor already has a unique outlet, those outlets coincide within tolerance with both ends of a root no longer than `2 * --endpoint-tolerance`, none coincides exactly, and `DSContArea` increases strictly into the root: the final source vertex is taken as the outlet (`root-source-order-trusted`). That residual class was empty for every basin of the preserved corpus and is reported so it can be retired.

**TRUST ASSUMPTION: a healthy isolated native root has no successor and no predecessor topology, so its orientation cannot be proven by endpoint coincidence. For those reaches only, the adapter trusts TauDEM/TDX native vertex order and treats the final LineString vertex as the outlet (`root-isolated-trusted`). This is a trust assumption, never an orientation proof.** The measured polarity of every proven reach on the preserved corpus is outlet first (for example 322,344 of 322,344 proven reaches in control `7020000010`), so this inherited convention probably names the head of an isolated root rather than its outlet, with an error up to the reach length; the control has 1,767 such roots. That is evidence for a maintainer adjudication, not a settled convention, and changing the rule alters previously compiled datasets. The report keeps trusted healthy isolated roots separate from endpoint-coincidence-proven links, predecessor-orientation-proven roots, and bounded near-degenerate resolutions, and separately identifies the trusted isolated roots that bear polygons.
