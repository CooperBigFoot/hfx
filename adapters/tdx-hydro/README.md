...
For every healthy native reach whose `DSLINKNO` names a successor, the adapter compares both endpoints of the reach with the distinct endpoint coordinates of that immediate native successor. Pairings within `--endpoint-tolerance` are admitted. If admitted pairings use exactly one reach endpoint, that exact source coordinate is the proven downstream endpoint (`endpoint-proven`). The successor endpoint need not be unique. TDX-Hydro contains many nonzero reaches shorter than twice the default `0.001`-degree tolerance, approximately 222 m, so one endpoint of a predecessor can legitimately coincide within tolerance with both endpoints of its short successor. This successor-side multi-match is accepted and reported; it does not make the predecessor's orientation ambiguous.

No admitted pairing is a fatal non-coincidence error. Orientation never consults source vertex order for a connected reach. On the preserved TDX-Hydro corpus the first vertex of every proven reach is its outlet, which is the opposite of the vertex-order convention earlier adapter revisions trusted for near-degenerate reaches. The `orient` report records the polarity histogram so a reviewer can check it per basin.

If admitted pairings use both endpoints of a healthy current reach, `DSContArea` must increase strictly downstream (a tie or a decrease is fatal) and the adapter conditions the admitted pairings on the successor's established upstream endpoint. For a non-root successor that endpoint is the one opposite the successor's already resolved outlet; for a root successor it comes from the predecessor evidence described below; for a degenerate successor it is the single stored coordinate. One remaining current endpoint is the outlet (`successor-side-proven`). This also covers a hairpin reach whose head lies near the successor's outlet, at any endpoint separation. Two remaining current endpoints mean the reach is at most `2 * --endpoint-tolerance` long and both ends sit near the junction; the endpoint that coincides exactly with the successor's upstream endpoint is the outlet (`exact-coincidence-proven`). Exact coincidence is a shared raster cell in the TauDEM-derived network, so it is source evidence rather than a distance heuristic; when neither or both endpoints coincide exactly the reach is fatal. No remaining current endpoint is fatal because the reach touches only the successor's outlet. The adapter does not snap an outlet to a successor, repair geometry, or use a spatial fallback.

**TRUST ASSUMPTION: a TDX-Hydro exactly degenerate reach has no directional axis, so its orientation is vacuous and is never counted as proven. Its single distinct source coordinate is treated as its downstream endpoint. A non-root degenerate reach must still coincide with its immediate native successor; non-coincidence remains fatal. This is a source-convention trust assumption, not spatial inference or an orientation proof.** A polygon-bearing degenerate reach uses that coordinate as its unit outlet. A degenerate root uses the same rule whether or not it has predecessors and is not counted as predecessor-orientation-proven, short-successor-resolved, reach-side-near-degenerate-resolved, or isolated.

For a healthy native root with predecessors, the root's upstream endpoint is collected before downstream traversal, so predecessor order cannot change the result. A predecessor with exactly one admitted pairing names the root endpoint it drains into, and so does any predecessor endpoint that coincides exactly with a root endpoint. One named endpoint is the root's upstream end and the opposite endpoint is the outlet (`root-predecessor-proven`). Evidence naming both root endpoints is fatal wherever it is consulted. When the evidence names no endpoint, a predecessor that matches both ends of the root cannot be conditioned and is fatal. In the residual case in which every predecessor already has a unique outlet, those outlets coincide within tolerance with both ends of a root no longer than `2 * --endpoint-tolerance`, none coincides exactly, and `DSContArea` increases strictly into the root, the basin polarity described next names the outlet (`root-basin-polarity`). That residual case was empty for every basin of the preserved corpus.

A healthy isolated native root has no successor and no predecessor topology, so its own endpoints prove nothing. Its outlet comes from the basin polarity, rule `basin-polarity-unanimous-proven-outlet-index-v1`: the adapter counts the outlet endpoint index of every reach proven by a unique endpoint coincidence and the outlet index named by every exact endpoint coincidence between two healthy reaches (an exact coincidence names the current reach's outlet and the successor's upstream end). When one index has a positive total and the other has zero, that index is the outlet of every isolated root (`root-basin-polarity`). When no reach is proven, or the counts are split between both indexes, any root that needs the polarity is fatal with a message naming the root and the counts; a basin without such a root still records the split or absent state. Reaches proven by coincidence never consult the polarity. The polarity is derived per basin from the basin's own reaches, so a source whose digitization runs the other way orients its isolated roots the other way. Every build report and `orient` report records the decision under `basin_polarity` with the rule id, the state, the chosen index, and both histograms. On the preserved corpus every one of the eight basins is unanimous at index 0, the first vertex; control `7020000010` counts 322,344 proven reaches and 655,197 exact coincidences at index 0 and none at index 1, and orients 1,767 isolated roots by the rule. The maintainer adopted this rule on 2026-09-04: the corrected outlets are the evidence-based ones. Earlier adapter revisions took the final vertex as the outlet of an isolated root, so the frozen 55-basin fabric keeps that older convention for its isolated roots and near-degenerate reaches, and the delivery of any extended fabric states that difference. The report keeps isolated roots separate from endpoint-coincidence-proven links, predecessor-orientation-proven roots, and bounded near-degenerate resolutions, and separately identifies the isolated roots that bear polygons.
...
The build report field `reach_side_near_degenerate_resolved_reach_native_linknos` lists every connected reach resolved after both of its endpoints were admitted, whichever rule settled it. The `orient` subcommand reports the per-class counts.

On the preserved corpus, the read-only `orient` preflight resolved every reach of `2020003440`, `2020065840`, `2020071190`, `4020050470`, `6020000010`, and control `7020000010`; `1020018110` and `5020049720` orient after their repeated `streamID` rows are taken as one dissolved unit each. For the control, the outlets of 5,425 units differ from the planetary revision `43a98aff` output by at most `0.00106` degrees: 5,340 exact-coincidence-proven reaches, one successor-side-proven reach, and 84 roots now proven from exact predecessor evidence, all of which that revision had resolved by trusting the final source vertex. Same-level graph edges are identical.
...
- `diagnostics.streamnet.isolated_root_count`
- `diagnostics.streamnet.isolated_root_native_linknos`
- `diagnostics.streamnet.polygon_bearing_isolated_root_count`
- `diagnostics.streamnet.polygon_bearing_isolated_root_native_linknos`
- `diagnostics.streamnet.orientation_tolerance`
- `diagnostics.streamnet.basin_polarity` (`rule_id`, `state`, `downstream_endpoint_index`, `proven_downstream_endpoint_index_counts`, `exact_coincidence_downstream_endpoint_index_counts`)
...
The adapter emits a warning for each nonzero clamp, including the layer, altered vertex count, and native IDs. After compilation it emits separate warnings for nonzero `contracted_edge_count`, `contracted_root_count`, `contracted_link_traversal_count`, and `polygonless_dropped_reach_count`. It emits three dedicated nonzero degenerate-reach warnings, each with a count and sorted native LINKNOs: all degenerate reaches, the polygon-bearing subset, and the polygon-less subset. It emits dedicated warnings with sorted native LINKNOs for nonzero `short_successor_resolved_reach_count` and `reach_side_near_degenerate_resolved_reach_count`; these fields exclude exact M4-S2 degeneracy and do not redefine M4-S3 area diagnostics. It also warns separately for nonzero trusted healthy isolated-root counts, including native LINKNO values, both for all healthy isolated native roots and for the polygon-bearing subset. The empirical area-unit decision, candidate-error ratio, visible raster-versus-vector divergence metrics, and complete streamnet summary are informational logs. Validity outside the exact two-identical-vertex source convention, native-successor non-coincidence, reach-side ambiguity that exact coincidence does not settle, indeterminate-root ambiguity beyond `2 * --endpoint-tolerance`, conflicting root predecessor evidence, and other contract failures raise errors rather than warnings.
...
uv run python build_adapter.py build \
  ...
  --created-at 2026-07-21T12:34:56+00:00
```

`--basins`, `--streamnet`, `--out`, `--report`, `--processing-basin-id`, and `--fabric-version` are required. `--created-at <timestamp>` is optional and accepts a timezone-aware ISO 8601 timestamp. It defaults to the current UTC time. Explicit values are converted to UTC in the manifest and build report, which permits reproducible dataset bytes when every other input is unchanged. Naive and invalid timestamps are rejected before source processing. `--endpoint-tolerance <degrees>` is optional and defaults to `0.001`; it controls which endpoint pairings are admitted for orientation and scales the `2 *` indeterminate-root limit. `--created-at` exists so a rebuild can reproduce an earlier dataset's manifest bytes for a byte-identity comparison, for example against a preserved control output.

Resolve reach orientation for one basin without compiling. The preflight reads only `basins.streamID` and the streamnet topology columns and endpoints, runs the same identity checks and compact topology step as `build`, and writes a JSON report with `outcome` (`resolved` or `refused`), per-class `resolution_counts`, the histogram of proven outlet endpoint indexes, the `basin_polarity` record, an `orientation_digest` over every unit's LINKNO, downstream LINKNO, and outlet coordinate, the streamnet diagnostics, and on refusal the exact message with its LINKNO pair. Repeated `streamID` rows are taken as one dissolved unit, as `build` does, and the report's `basins_identity_dissolve` records that assumption; the interior-overlap check of the dissolve needs geometry and is left to `build`. Orientation compares `DSContArea` only by order, so the preflight uses raw values without unit inference. The process exits `1` on refusal after writing the report:

```bash
uv run python build_adapter.py orient \
  --basins /path/to/7020000010-basins.gpkg \
  --streamnet /path/to/7020000010-streamnet.gpkg \
  --processing-basin-id 7020000010 \
