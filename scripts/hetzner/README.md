...
Compile and extension runbook: [compile the seven absent basins with the corrected adapter, prove the planetary control rebuild byte for byte and the corrected control against the maintainer-adjudicated outlet difference, extend the frozen 55-basin artifact, and preserve everything before exact-resource teardown](RUNBOOK-tdx-hydro-seven-basin-compile.md). Its helpers are `verify-compile-runbook.sh` (contract drift), `verify-campaign-inputs.sh` (evidence root, opaque credential file, hcloud context), `price-preflight.sh` (current-price cost projection and gates), `compare-dataset-trees.sh` (SHA-256 comparison of two dataset trees), `seven-basin-control-adjudication.json` (the pinned adjudicated difference between the corrected and planetary control builds), and `adapters/tdx-hydro/compare_unit_outlets.py` (per-unit comparison of two builds of one basin against that record).
