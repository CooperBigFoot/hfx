...
This campaign consumed the approved bounded Hetzner lifecycle for Effort #195 and reached exact-resource teardown on 2026-09-07. No production lifecycle remains authorized. The fences below are retained as execution history; they are not permission to launch again. The executable guard merged in PR #244 refuses this consumed production authority before any driver effects. The historical procedure follows: the campaign builds two control outputs for basin `7020000010`, proves the planetary rebuild byte for byte against the preserved control, proves the corrected build against the maintainer-adjudicated difference record, compiles the seven absent basins with the corrected adapter, extends the frozen 55-basin artifact with every newly compiled basin, attempts strict whole-dataset validation, preserves everything off the VM with digests, and tears down by exact resource identity.
  ...
  "lifecycle_ledger": {
    ...
    "rehearsal_authority": {
      ...
      "runs": [
        ...
        {"date": "2026-09-05", "provisioning_request": "2026-09-05T16:14:27Z", "provisioning_request_epoch": 1788624867, "zero_footprint": "2026-09-05T16:39:13Z", "server_id": 164691973, "volume_id": 106798609, "server_type": "cx23", "volume_size_gb": 10, "ground_truth_ref": "a8d4ef29b9fd95d465cbff27eefc239a5717c283", "result": "passed", "strict_validation": "passed", "control_gates": {"corrected_versus_reference": "identical", "planetary_versus_reference": "created-at-only", "adjudicated": "accepted", "outlet_differences": 0}, "workload_dispatched": true, "estimated_cost_eur": 0.01}
      ]
    },
    "current_authority": {"status": "consumed", "maintainer": "Nicolas Lazaro", "date": "2026-09-04", "lifecycles": 1, "record": "https://github.com/CooperBigFoot/hfx/pull/234", "precondition": "a lifecycle-result.json with result passed under the rehearsal evidence root for campaign-rehearsal", "limits": "one ccx33 in fsn1, one 600 GB volume, under 72 hours from the provisioning request, under EUR 40.00 projected and actual, exact-resource teardown"},
    "completed": {"date": "2026-09-05", "provisioning_request": "2026-09-05T21:18:04Z", "provisioning_request_epoch": 1788643084, "server_id": 164714525, "volume_id": 106799807, "workload_dispatched": true, "compiles_succeeded": "2026-09-06T04:00:11Z", "aborted": "2026-09-06T04:07:27Z", "cause": "final-record-write-exit-126", "baseline_pulled": true, "preservation_target": "bucket-only", "workstation_dataset_copies_to_delete_after_bucket_verification": [{"path": "off-vm/campaign/basin-outputs", "approximate_bytes": 29000000000, "origin": "section 11 fence, complete"}, {"path": "salvage/control-builds", "approximate_bytes": 8000000000, "origin": "trap salvage, complete"}, {"path": "salvage/campaign/basin-outputs", "approximate_bytes": 20000000000, "origin": "trap salvage, partial; stopped by the operator"}], "result": "workload-completed-driver-failed", "zero_footprint": "2026-09-07T11:39:49Z", "resumed_at": "2026-09-06T15:16:41Z", "resume_revision": "a1ebc116d326c9b4b192dcc942f2a42b40d9b2b2", "strict_validation": "passed", "driver_exit_code": 126, "driver_failed_at": "2026-09-07T11:39:25Z", "elapsed_hours": 38.3625, "conservative_estimated_gross_eur": "11.3708598205", "invoiced_actual_gross_eur": null, "authority_status": "consumed", "evidence_record": "scripts/hetzner/CAMPAIGN-tdx-hydro-extension.json", "earlier_interruption_cause": "awk-default-format-printed-baseline-byte-sum-as-exponent"}
  },
...
### Third lifecycle interruption and resume on 2026-09-06

The production lifecycle under the current authority was requested on 2026-09-05 at 21:18:04Z (provisioning request epoch `1788643084`, server `164714525`, volume `106799807`). Provisioning, bootstrap, convergence, initialization, the corpus transfer, the adopting acquisition, both control builds with their gates, and the per-basin compiles all passed: the compiles of all seven absent basins succeeded at 04:00:11Z on 2026-09-06. The section 12 baseline pull then aborted at 04:07:27Z with the baseline fully pulled (6 objects, 114063230627 bytes): the remote byte check summed the object sizes with `awk '{s+=$1} END {print s}'`, the Debian 12 `mawk` printed that sum as `1.14063e+11`, and `test -eq` rejected it with `integer expression expected`. The rehearsal and the dry run carry tiny data, so no sum there ever reached 2^31. The lifecycle was held for a resume at the `baseline` stage with the same server and volume; the interim conservative estimate was about EUR 2.04 at 6.7 hours of the 72-hour ceiling. The section 12 pull now skips every object whose local size matches the listing, so the resume re-downloads nothing. Every byte total is now summed in shell integer arithmetic (sections 4, 12, and 14, and the rehearsal preparation), and the composer refuses a fence that prints an accumulated awk sum through awk's default number format. The completed resume, final driver failure, zero-footprint proof and conservative final estimate are recorded below.

### Production lifecycle consumed on 2026-09-07

The production lifecycle requested at 2026-09-05T21:18:04Z used server `164714525` and volume `106799807`. All seven absent processing basins and the control compiled. After the baseline byte-sum interruption, the same resources resumed at revision `a1ebc116d326c9b4b192dcc942f2a42b40d9b2b2`. The extension workload finished at 2026-09-07T11:17:46Z with exit 0 and strict validation passed. Final record generation then failed with driver exit 126 at 11:39:25Z. Exact-resource teardown reached zero footprint at 11:39:49Z. The original campaign record is empty and no passing lifecycle-result was produced.

The [campaign record](CAMPAIGN-tdx-hydro-extension.md) separates the completed workload from the failed driver and the resolved small-manifest observation, while retaining the stale bucket-state snapshot as historical operational evidence. The elapsed interval was 38.3625 hours; conservative estimated gross spend was EUR 11.3708598205, without an invoice claim. The operator added 55 GB of volume swap during validation. The two earlier consumed entries remain historical; `lifecycle_ledger.completed` records this third production lifecycle and `current_authority.status` marks its authority consumed. Those record fields grant no future authority. The inherited `workstation_dataset_copies_to_delete_after_bucket_verification` field preserves a historical inventory only; it grants no deletion authority. No automated deletion is permitted, and any later cleanup requires a separate explicit decision. The executable guard merged in PR #244 refuses the consumed authority. Read-only `authority-is-pinned` and scope checks pass; `authority-is-current` refuses. Prose and record fields alone are not a launch guard.
...
if test -n "${HFX_CAMPAIGN_CONTRACT:-}"; then
  ...
  CONTRACT_RESOLVED_FROM=$(jq -r '.resolved_from // empty' <<<"$CAMPAIGN_CONTRACT_JSON")
  if test -n "$CONTRACT_RESOLVED_FROM"; then
    case "$CONTRACT_RESOLVED_FROM" in scripts/hetzner/*-contract.json) ;; *) exit 1 ;; esac
    test -f "$CONTRACT_RESOLVED_FROM"
    missing_contract_fields=$(jq -r --slurpfile tracked "$CONTRACT_RESOLVED_FROM" '[paths] as $present | [$tracked[0] | paths | select(.[0] != "derived_at_preparation")] - $present | map(map(tostring) | join(".")) | join(" ")' <<<"$CAMPAIGN_CONTRACT_JSON")
    if test -n "$missing_contract_fields"; then
      printf 'hfx: refusing: resolved contract %s lacks %s, present in the tracked %s; an older prepare-rehearsal-campaign.sh wrote it, so rerun that tool with --re-resolve\n' "$HFX_CAMPAIGN_CONTRACT" "$missing_contract_fields" "$CONTRACT_RESOLVED_FROM" >&2
      exit 1
    fi
  fi
else
...
if test "$(contract_value '.requires_passing_dry_run')" = true; then
  ./scripts/hetzner/verify-compile-runbook.sh --evidence-root "$HFX_CAMPAIGN_EVIDENCE" --check dry-run-passed
fi
if test "$CONTROL_REFERENCE" = preserved-off-vm; then
...
A resolved rehearsal contract carries `resolved_from`, the tracked record it was resolved from. The shell compares the two before it reads any field: a resolved contract written by an older `prepare-rehearsal-campaign.sh` lacks fields added to the tracked record since (rehearsal run 4's setup lacked `requires_passing_dry_run`, and the section 5 read of that field ended the driver with an opaque jq error), so the shell refuses up front, names the resolved contract and the missing fields, and points at `prepare-rehearsal-campaign.sh --re-resolve`, which regenerates the resolved contract from the tracked record while keeping the derived values. Under `preserved-off-vm` the preserved control is a shared input of the evidence root (`off-vm/control-builds/planetary/<control>`); under `vm-planetary-build` it is written per campaign under the evidence directory this shell creates, so a rerun never meets the previous run's reference.
...
bash scripts/hetzner/test-prepare-rehearsal-campaign.sh
bash scripts/hetzner/test-campaign-dry-run.sh
...
for check in scope-permits-compilation ceilings-and-kill-switches control-hotpatch-is-pinned control-digests-are-pinned control-adjudication-is-pinned baseline-is-pinned authority-is-pinned rehearsal-record-is-pinned; do
  bash scripts/hetzner/verify-compile-runbook.sh --check "$check"
done
git diff --check
```

The static checklist uses `authority-is-pinned` to audit historical provenance. Separately require `authority-is-current` to refuse this consumed production contract. The operational preflight retains `authority-is-current`; a static provenance pass grants no launch authority. The independently reviewed consumed-authority guard in PR #244 supplies these checks; the current production contract must continue to refuse execution.
