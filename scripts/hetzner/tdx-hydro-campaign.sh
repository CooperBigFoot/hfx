...
parse_final_headers() {
    ...
    [[ -n "$header_status" && "$header_etag_count" -le 1 &&
       "$header_content_length_count" -le 1 ]] &&
        { [[ "$header_etag_count" -eq 0 ]] || strong_etag_is_safe "$header_etag"; }
}
...
validate_acquisition_report() {
    ...
    "$JQ" -e --arg basin "$basin_id" --arg product "$product" '
      (keys | sort) == ["processing_basin_id","product","retry_count",
                       "schema_version","transfers"] and
      .schema_version == 1 and .processing_basin_id == $basin and .product == $product and
      (.retry_count | type == "number" and floor == . and . >= 0) and
      (.transfers | type == "array" and length > 0) and
      .retry_count == ((.transfers | length) - 1) and
      ([.transfers[].attempt] | . as $attempts |
       all(range(1; length); $attempts[.] == ($attempts[. - 1] + 1))) and
      all(.transfers[];
        (keys | sort) == ["attempt","average_bytes_per_second","http_status","mode",
                         "network_bytes","result","time_total_seconds"] and
...
emit_acquisition_summary() {
    ...
    local status=$3
    local report=$4
    local retry=0 network=0 time=0 speed=0
    ...
    printf 'hfx: acquisition product=%s-%s status=%s retry_count=%s last_network_bytes=%s last_time_total_seconds=%s last_average_bytes_per_second=%s\n' \
        "$basin_id" "$product" "$status" "$retry" "$network" "$time" "$speed" >&2
}
...
acquire_product() {
    ...
    if [[ "$result" != succeeded ]]; then
        append_transfer_report "$report" "$report_tmp" "$basin_id" "$product" \
            "$attempts" "$result" || return 1
        fail_product "$basin_id" "$stage" "$attempts" \
            "$([[ "$result" == curl_failed ]] && printf 'transfer interrupted; retry from byte zero' || printf 'download provenance or size verification failed')"
        emit_acquisition_summary "$basin_id" "$product" failed "$report"
        return
    fi
    if ! verify_download "$partial" "$inspect"; then
        append_transfer_report "$report" "$report_tmp" "$basin_id" "$product" \
            "$attempts" integrity_failed || return 1
